...
async def critical_error(interaction: nextcord.Interaction, errorname: str, errorcause: str, traceback: str):
    ...
    embed = nextcord.Embed(title=kira_language.get_text('ko', "error-embed-title"),
                           description=f"{kira_language.get_text('ko', 'error-embed-description-part-1')}{error_time}{kira_language.get_text('ko', 'error-embed-description-part-2')}",
...
async def success(interaction: nextcord.Interaction, embed_desc: str, image_url: str = None):
    embed = nextcord.Embed(title=kira_language.get_text('ko', "embed-title-success"), description=embed_desc,
...
async def error(interaction: nextcord.Interaction, embed_desc: str, image_url: str = None):
    embed = nextcord.Embed(title=kira_language.get_text('ko', "embed-title-error"), description=embed_desc,
...
async def developing(interaction: nextcord.Interaction):
    embed = nextcord.Embed(title=kira_language.get_text('ko', "embed-title-developing"),
                           description=kira_language.get_text('ko', "embed-title-developing-info"),
