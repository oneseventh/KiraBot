...
class MemoCommand(commands.Cog):
    ...
    async def memo(self, interaction: Interaction,
                   action: int = nextcord.SlashOption(name="행동", choices={"📖 메모 읽기": 1, "📝 메모 쓰기": 2, "🔥 메모 삭제": 3},
        ...
        if action == 1:
            ...
            embed = nextcord.Embed(title=kira_language.get_text('ko', "memo-embed-title"),
                                   description=kira_language.get_text('ko', "memo-embed-description"),
                                   color=nextcord.Color.random())
            embed.set_author(name=f"Request by {interaction.user}", icon_url=interaction.user.avatar)
            embed.add_field(name=kira_language.get_text('ko', "memo-embed-field-memo-num"),
                            value=f"``#{result[0]}``", inline=True)
            embed.add_field(name=kira_language.get_text('ko', "memo-embed-field-memo-name"),
                            value=f"``{result[3]}``", inline=True)
            embed.add_field(name=kira_language.get_text('ko', "memo-embed-field-memo-time"),
                            value=f"``{result[1]}``", inline=True)
            embed.add_field(name=kira_language.get_text('ko', "memo-embed-field-memo-content"),
                            value=f"```{result[4]}```", inline=False)
            embed.set_footer(
                text=f"Developed by {kira_language.get_text('ko', 'PART1_DEVELOPER_NAME')}",
                icon_url=f"{kira_language.get_text('ko', 'PART1_DEVELOPER_PROFILE_URL')}")
...
class MemoButton(nextcord.ui.View):
    ...
    @nextcord.ui.button(label=kira_language.get_text('ko', 'memo-button-text-download'),
    ...
    @nextcord.ui.button(label=kira_language.get_text('ko', 'memo-button-text-remove'),
...
async def download_memo(interaction: Interaction, user_id: int, memo_name: str):
    ...
    await interaction.user.send(f"``{memo_name}-{user_id}.txt`` "
                                f"{kira_language.get_text('ko', 'memo-private-dm-message')}",
                                files=[nextcord.File(file_path)])
    await alert.success(interaction, kira_language.get_text('ko', 'memo-download-success'))


class WriteMemo(nextcord.ui.Modal):
    def __init__(self):
        super().__init__(
            kira_language.get_text('ko', "memo-write-modal-title")
        )
    ...
    async def callback(self, interaction: nextcord.Interaction) -> None:
        if check_memo(interaction.user.id, self.memo_name.value):
            await alert.error(interaction, kira_language.get_text('ko', "memo-error-name-already-exist"))
            return
        else:
            conn = sqlite3.connect('././memo.db', isolation_level=None)
            c = conn.cursor()
            c.execute("INSERT INTO memo (user_id, memo_name, content) VALUES (?, ?, ?)",
                      (interaction.user.id, self.memo_name.value, self.memo_content.value))
            conn.commit()
            conn.close()
            await alert.success(interaction, f"``{self.memo_name.value}``"
                                             f" {kira_language.get_text('ko', 'memo-success-write')}\n"
                                             f"**{kira_language.get_text('ko', 'memo-embed-field-memo-content')}**"
