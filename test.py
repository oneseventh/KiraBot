...
'''
@client.command()
async def set_welcome(ctx, channel: nextcord.TextChannel):
...
#로그 저장할 채널을 설정하는 명령어
@client.command()
async def log_channel(ctx, channel: nextcord.TextChannel):
    path = "C:\\Users\\Administrator\\Desktop\\src\\discord-bot\\ScienceBot\\setting.txt"
    global channel_id
    ...
    if channel_id != 0:
        ...
        embed=nextcord.Embed(title=f"{channel.name} 채널에 로그를 저장합니다.", description=f"한번 선택한 채널은 바꿀 수 없습니다\n(변경기능 개발중!)", color=0x5947FF)
...
async def on_raw_message_delete(payload):
    ...
    now = time.localtime()
    channel = client.get_channel(channel_id)
    embed=nextcord.Embed(title="님의 메세지를 삭제하였습니다", description=f"메세지가 삭제된 채널 <#{payload.cached_message.channel.id}>", color=0x5947FF)
...
async def on_message_edit(before, after):
    ...
    now = time.localtime()
    channel = client.get_channel(channel_id)
    embed=nextcord.Embed(title="님의 메세지를 수정하였습니다", description=f"메세지가 수정된 채널 <#{before.channel.id}>", color=0xE189FC)
...
async def on_guild_channel_create(channel):
    ...
    embed=nextcord.Embed(title=f"<{channel}> 채널이 생성되었습니다", color=0x00FF22)
...
async def on_guild_channel_delete(channel):
    log_channel = client.get_channel(channel_id)
    embed=nextcord.Embed(title=f"<{channel}> 채널이 삭제되었습니다", color=0xFF0000)
    embed.add_field(name="IDs", value=f"```diff\n+ {channel.id} (role) \n+ {int(channel.guild.id)} (guild)```", inline=False)
    embed.set_footer(text="Developed by 동건#3038")
    await log_channel.send(embed=embed)
...
async def on_member_join(member):
    ...
    embed.add_field(name="서버에 입장한 시일",value=f"``{member.joined_at}``\n(UTC+00:00 기준)", inline=True)
...
async def on_member_update(before, after):
    ...
    if(before.display_name != after.display_name):
        embed.add_field(name="서버 닉네임", value=f"{before.display_name} -> {after.display_name}", inline=False)
        checking = True
    if checking == True:
...
async def on_user_update(before, after):
    ...
    checking = False
    if before.name != after.name:
        embed.add_field(name="프로필 닉네임", value=f"{before.name} -> {after.name}", inline=False)
        checking = True
    if before.discriminator != after.discriminator:
        embed.add_field(name="프로필 디스택", value=f"{before.discriminator} -> {after.discriminator}", inline=False)
        checking = True
    if before.avatar != after.avatar:
        embed.add_field(name="프로필 사진", value=f"``{before.display_avatar}`` -> ``{after.display_avatar}``", inline=False)
        checking = True
    if checking == True:
